...
import com.xtxk.cn.controller.alarm.AlarmDefineController;
 ...
 * @Description 基于hutool的http客户端组件
 * @Date create in 2022-07-16 16:03
 * @Modified by
 */

public class HutoolHttpClient {

    private static final Logger LOGGER = LoggerFactory.getLogger(AlarmDefineController.class);
     ...
     * @return
     */
    private cn.hutool.http.HttpResponse innerHttpRequest2(String url, JSONObject param, BiFunction<String, JSONObject, cn.hutool.http.HttpResponse> request) {
        Objects.requireNonNull(url, "请求URL不能为空！");
        LOGGER.debug("hutool client 请求URL： >> {}", url);
        LOGGER.debug("hutool client 请求参数： >> {}", param);
        // 如果请求参数为空，构建空的请求参数
    ...
    public cn.hutool.http.HttpResponse httpGet(String url, JSONObject param) {
        return innerHttpRequest2(url, param, (u, p) -> {
            String paramStr = MapUtil.join(p, "&", "=", false);
            return HttpUtil.createGet(u + "?" + paramStr).execute();
        });
    }
